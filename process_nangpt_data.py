...
def process_line(line, printraw):
    #print(line)
    #parts = [part for part in line.strip().split(',') if part and part != "ipex"]
    parts = [part for part in line.strip().split(',') if part and part != "nonipex"]
    common_elements = parts[:7]

    first_instance = parts[7]
    #print(first_instance)
    token_value = extract_value(first_instance, 'token')
    bs_value = extract_value(first_instance, 'BS')

    inftime_values = [extract_value(part, 'inftime') for part in parts[7:] if 'inftime' in part]
